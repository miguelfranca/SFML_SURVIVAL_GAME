...
bool GameLoop(mywindow &w) {
	...
	while (!(sf::Keyboard::isKeyPressed(sf::Keyboard::Escape))) {
		...
		sf::Joystick::update();
		if ((space_released && (sf::Keyboard::isKeyPressed(sf::Keyboard::Space))) || sf::Joystick::isButtonPressed(0, 9)) {
		...
		if (!game_pause) {
			...
			sf::Joystick::update();
			if ((sf::Keyboard::isKeyPressed(sf::Keyboard::A) || sf::Keyboard::isKeyPressed(sf::Keyboard::Left) || sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::X) / 100.f < -0.2f) && (player.getX() >= (player.getSize().x / 2) + boundary_x))			
				player.move(-1., 0.);

			if ((sf::Keyboard::isKeyPressed(sf::Keyboard::D) || sf::Keyboard::isKeyPressed(sf::Keyboard::Right) || sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::X) / 100.f > 0.2f) && (player.getX() <= screenWidth - player.getSize().x / 2 - boundary_x))
				player.move(1., 0.);

			if ((sf::Keyboard::isKeyPressed(sf::Keyboard::W) || sf::Keyboard::isKeyPressed(sf::Keyboard::Up) || sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::Y) / 100.f < -0.2f) && (player.getY() >= (player.getSize().y / 2) + boundary_y))				player.move(0., -1.);

			if ((sf::Keyboard::isKeyPressed(sf::Keyboard::S) || sf::Keyboard::isKeyPressed(sf::Keyboard::Down) || sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::Y) / 100.f > 0.2f) && (player.getY() <= screenHeight - player.getSize().y / 2 - boundary_x))
				player.move(0., 1.);


			if(player.dead_frame_count >= Player::dead_frames_total - 1)
				return true; // when dead make one more loop cycle, to draw the dead texture and only then (now) leave

			if(player.dead_frame_count) //if already dying
				++player.dead_frame_count;
			else{
				for (int i = 0; i < max_bots; i++)
					if (player.collide(bots[i]))
						++player.dead_frame_count;			
			}
			...
			//////////// JOYSTICK SHOTS CONTROLLER //////////
			if ((abs(sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::Z)) > 7.f || (abs(sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::R)) > 7.f)) && (frame_count_shot >= player.getPeriodD() / delay)) {
				player.setUpShot(sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::Z) * screenWidth, sf::Joystick::getAxisPosition(0, sf::Joystick::Axis::R) * screenHeight, player.getX(), player.getY());
				w.bullet_sound.play();
				frame_count_shot = 0;
			}
...
bool GameOver(mywindow &w) {
	...
	while (1) {
		...
		else if (sf::Keyboard::isKeyPressed(sf::Keyboard::Space) || sf::Joystick::isButtonPressed(0, 1)) {
			break;
		}
	}
	w.window.clear(sf::Color::Black);
