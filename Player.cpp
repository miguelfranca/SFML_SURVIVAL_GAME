...
void Player::draw(sf::RenderWindow& window)
{
	...
	int index = (int)std::round((atan2(look_position.y - getY(), look_position.x - getX()) + PI) * 8.f/(2.f*PI)) % 8;
	if( !dead_frame_count ){
		shape.sf::RectangleShape::setTexture(shape_default[index].getTexture());
		shape.draw(window); 
	}
	else{
		MyShape dead_shape;
